...
var createMainPanel = function () {
    ...
    mainPanel.port.on('saveSettings', function(basicSettings) {
        settingsHandler.saveBasicSettings(basicSettings);
    });
    mainPanel.port.on('exportToExcel', function() {
        controllers.uiController('exportToExcel', {
            mainPanel: mainPanel
        });
    });
};
