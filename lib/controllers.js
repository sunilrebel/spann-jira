...
exports.pageController = function (message, params) {
    switch (message) {
        ...
        case 'processingComplete' :
            storageSystem.addJiraData(params.data.spannQueryCounter, params.data.jiraData);
            break;
...
exports.listController = function (message, params) {
    switch (message) {
        ...
        case 'processingComplete' :
            storageSystem.addJiraList(params.data.spannQueryCounter, params.data.jiraList);
            exports.listController('startJiraProcessing', params);
            break;

        case 'startJiraProcessing' :
            var jiraList = params.data.jiraList;
            for (var i=0; i<jiraList.length; i++) {
                exports.pageController('start', {url: "https://jira.mongodb.org/browse/" + jiraList[i]});
                if (i==4)
                    break;
            }
            break;
