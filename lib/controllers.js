...
exports.uiController = function (message, params) {
    switch (message) {
        ...
        case 'startContentProcessing' :
            var baseUrl = settingsHandler.getSettings().baseJiraUrl;

            if (settingsHandler.getSettings().isEnabled && baseUrl) {
                var jiraUrlList = [];
                var jiraList = params.data.toString().match( /( |^|\s|\()[a-zA-z]+-[\d]+/gi );
                for (var i in jiraList) {
                    var url = baseUrl + "browse/" + jiraList[i].trim();
                    jiraUrlList.push(url);
                }
                exports.uiController('startProcessing', {
